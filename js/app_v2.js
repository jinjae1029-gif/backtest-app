// app.js
import { runSimulation } from './logic.js?v=debug2';
...
document.addEventListener('DOMContentLoaded', () => {
            ...
            if (isTradingSheet) {
                ...
                if (defaults) {
                    try {
                        ...
                        // Weights (Need to check if p.safe.weights exists)
    ...
    const getNextRebalanceDate = () => {
        ...
        if (typeof window.lastFinalState !== 'undefined') {
            ...
            if (isPending) {
                // If pending, rebalance is Day 10 (Tomorrow). Next Cycle Start is +1 after that.
                daysToWait = 2;
            } else {
    ...
    const openInjModal = (type) => {
        ...
        if (type === 'SEED') {
            ...
            if (!isNaN(d.getTime())) {
                ...
                nextBizDay = d.toISOString().split('T')[0];
            } else {
                nextBizDay = new Date().toISOString().split('T')[0];
            }
    ...
    if (btnUseDefaults) {
        btnUseDefaults.addEventListener('click', () => {
            ...
            localStorage.setItem('tradingSheetDefaults', JSON.stringify(defaults));
            alert("현재 설정이 '매매 시트' 기본값으로 저장되었습니다.");
...
function updateTierInputs() {
    const isRealTier = document.getElementById('toggleRealTier').checked;
    const safe8 = document.getElementById('safeWeight8');
    const off8 = document.getElementById('offWeight8');
    ...
    // Auto-fill Real Tier Defaults if checked (User Convenience)
    if (isRealTier) {
        const safeDefaults = [0, 20, 20, 20, 20, 20, 0, 0];
        const offDefaults = [0, 0, 20, 20, 20, 20, 20, 0];

        const safeInputs = document.querySelectorAll('#safeWeights input');
        safeInputs.forEach((inp, i) => { if (inp) inp.value = safeDefaults[i]; });

        const offInputs = document.querySelectorAll('#offWeights input');
        offInputs.forEach((inp, i) => { if (inp) inp.value = offDefaults[i]; });
    }
}
...
function renderOrderSheet(result) {
    ...
    // Check Rebalance Day
    const pendingRebalance = s.pendingRebalance;
    const isRebalanceDay = pendingRebalance !== null && pendingRebalance !== undefined;
    ...
    s.holdings.forEach((h, i) => {
        ...
        // Check standard timecut logic: if next day (daysHeld+1) >= limit, Sell MOC.
